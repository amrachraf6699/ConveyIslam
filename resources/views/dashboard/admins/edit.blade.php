...
@section('title', 'تحرير مسؤول ')
@section('content')
    <div class="max-w-6xl mx-auto">
        <header class="relative z-10 mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl md:text-4xl font-bold text-white mb-2">تحرير مسؤول </h1>
                    <p class="text-white opacity-80 italic">قم بملء النموذج أدناه لتحرير المسؤول إلى النظام.</p>
                </div>
            </div>
        </header>
        ...
        <div class="relative z-10">
            <form id="languageForm" action="{{ route('dashboard.admins.update', $admin) }}" method="POST" class="space-y-8">
                @csrf
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <div>
                            ...
                            <input type="text" id="englishName" name="name" required
                                ...
                                placeholder="الاسم" dir="rtl" value="{{ old('name', $admin->name) }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="email" id="nativeName" name="email" required
                                class="p-3 rounded-xl bg-white bg-opacity-10 border border-yellow-400 border-opacity-20 text-white w-full focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-50"
                                placeholder="البريد الإلكتروني" dir="ltr" value="{{ old('email', $admin->email) }}">
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <input type="text" id="nativeName" name="password"
                                class="p-3 rounded-xl bg-white bg-opacity-10 border border-yellow-400 border-opacity-20 text-white w-full focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-opacity-50"
                                placeholder="أدخل كلمة السر" dir="rtl">
                            <p class="text-sm text-gray-300 mt-2 leading-relaxed">
                                يمكنك ترك هذا الحقل فارغًا إذا كنت لا ترغب في تغيير كلمة السر الحالية.:
                            </p>
                        </div>
                    </div>
                </div>
